...
export const List: FunctionComponent<Props> = ({ tasks }) => (
  <div className="p-4">
    <div className="flex justify-between items-center">
      <h1 className="text-3xl mb-2">Task List</h1>
      <Link
        href="/tasks/create"
    ...
    >
      <thead className="w-full text-xs uppercase font-light text-gray-700 bg-gray-200 py-2 px-4">
        <tr>
          ...
          <th>authoredOn</th>
          <th>lastModified</th>
          <th>requesterPractitionerRole</th>
          <th>requestedPerformers</th>
          <th colSpan={2} />
        </tr>
      </thead>
      <tbody className="text-sm divide-y divide-gray-200">
        {tasks &&
          ...
          tasks.map(
            (task) =>
              task["@id"] && (
                <tr className="py-2" key={task["@id"]}>
                  <th scope="row">
                    <ReferenceLinks
                      items={{
                        href: getItemPath(task["@id"], "/tasks/[id]"),
                        name: task["@id"],
                      }}
                    />
                  </th>
                  ...
                  <td>
                    <ReferenceLinks
                      items={task["requesterPractitionerRole"].map(
                        (ref: any) => ({
