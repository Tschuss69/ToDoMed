...
interface Props {
  codeableconceptcategorys: CodeableConceptCategory[];
}

export const List: FunctionComponent<Props> = ({
  codeableconceptcategorys,
}) => (
  <div className="p-4">
    <div className="flex justify-between items-center">
      <h1 className="text-3xl mb-2">CodeableConceptCategory List</h1>
      <Link
        href="/codeableconceptcategorys/create"
    ...
    >
      <thead className="w-full text-xs uppercase font-light text-gray-700 bg-gray-200 py-2 px-4">
        <tr>
          ...
          <th>category</th>
          <th>categoryHash</th>
          <th>categorycodeableconcept</th>
          <th>parent</th>
          <th>codeableconcepts</th>
          <th>codeableConcepts</th>
          <th>categoryCodeableConcept</th>
          <th colSpan={2} />
        </tr>
      ...
      <tbody className="text-sm divide-y divide-gray-200">
        {codeableconceptcategorys &&
          codeableconceptcategorys.length !== 0 &&
          codeableconceptcategorys.map(
            (codeableconceptcategory) =>
              codeableconceptcategory["@id"] && (
                <tr className="py-2" key={codeableconceptcategory["@id"]}>
                  <th scope="row">
                    <ReferenceLinks
                      items={{
                        href: getItemPath(
                        ...
                        name: codeableconceptcategory["@id"],
                      }}
                    />
                  </th>
                  ...
                  <td>
                    <ReferenceLinks
                      items={codeableconceptcategory["parent"].map(
                        (ref: any) => ({
                          href: getItemPath(
                            ref,
                            "/codeableconceptcategorys/[id]"
